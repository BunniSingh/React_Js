...
const Restaurants = () => {
    ...
    function searchWithNaem(e) {
        setInput(e.target.value);
    }

    function searchWithRating(e) {
        setRating(e.target.value);
    }
    return (
        <>
            <h1>Our Favourite Restaurants</h1>
            <div className={style['filter-box']}>
                <div className={style.byName}>
                    <label htmlFor='search'>Search your favourite restaurants</label>
                    <input type="text" onChange={searchWithNaem} className={style.search} placeholder="Search..." />
                </div>
                <div className={style.byName}>
                    <input type="number" onChange={searchWithRating} className={style.rating} />
                    <label htmlFor='rating'>Search with ratings</label>
                </div>
            </div>
            <div className={style.container}>
                {
                    restaurantList.filter((data) =>{
                        if((data.name.toLowerCase().includes(inputVal.toLowerCase().trim()) || data.address.toLowerCase().includes(inputVal.toLowerCase().trim) || data.type_of_food.toLowerCase().includes(inputVal.toLowerCase().trim())) && Number(data.rating) >= Number(ratingVal)){
                            return true;
                        }else{
                            return false;
                        }
                    })
                    .map((restaurantData, idx) =>{
                        return <RestaurantCard key={idx} {...restaurantData} />
                    })
                }
            </div>
